...
test.describe("Homepage", () => {
  ...
  test("should display navbar with all elements", async ({ page }) => {
    ...
    if (viewport && viewport.width >= 768) {
      ...
      await expect(
        page.locator("nav").getByRole("link", { name: "Code" })
      ).toBeVisible();
      await expect(
        page.locator("nav").getByRole("link", { name: "SKills" })
      ).toBeVisible();
    } else {
  ...
  test("should display terminal component", async ({ page }) => {
    ...
    await expect(page.locator("div.text-xs.font-mono")).toContainText(
      "~/homelab/playbooks/baseline.yml"
    );
  });
  ...
  test("should display skills section", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: "Linux Platform & Virtualization" })
    ).toBeVisible();
    await expect(
      page.getByRole("heading", { name: "Automation & IaC" })
    ).toBeVisible();
    await expect(
      page.getByRole("heading", { name: "Databases" })
    ).toBeVisible();
    await expect(
      page.getByRole("heading", { name: "Reliability & Observability" })
    ).toBeVisible();
  });
  ...
  test("should display footer with social links", async ({ page }) => {
    ...
    await expect(
      page.locator("footer").getByRole("button", { name: /GitHub/i })
    ).toBeVisible();
    await expect(
      page.locator("footer").getByRole("button", { name: /LinkedIn/i })
    ).toBeVisible();
    await expect(
      page.locator("footer").getByRole("button", { name: /Contact/i })
    ).toBeVisible();
  });
